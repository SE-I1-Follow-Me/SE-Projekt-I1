...
Die serverseitigen Komponenten der Anwendung werden auf einem Server der HTW-Dresden betrieben. Die Anwendung in Form einer Android-App wird nach Fertigstellung in dem offiziellen Google-Play-Store zum Download bereit stehen. 
...
- Das Essence Manager Tool wurde nicht von Anfang an verwendet. Dies hat das Problem mit sich gebracht, dass zwar einzelne Aufgaben vergeben werden konnten, auf Managementebene jedoch der allgemeine Überblick gefehlt hat. Da wir uns hier jedoch an den Vorlesungsfolien zum OpenUP orientierten, konnten wir dieses Problem mit Mehraufwand umgehen. 
- Die Use-Cases wurden nicht agil genug entwickelt. So hat die Erstellung der Use-Cases bedeutend länger gedauert als es müsste, da hier nicht agil auf den Arbeitsaufwand eingegangen wurde. Mit einer Anpassung von Human Ressources hätte diesem Problem entgegengewirkt werden können.
- Der Terminfindungsprozess stellte gelegentlich ein Problem dar, da bei einer Gruppe aus sieben Studierenden häufig zu Überschneidungen individueller Zeitplanungen kommt. Hier kann ein Terminfindungstool Abhilfe schaffen.