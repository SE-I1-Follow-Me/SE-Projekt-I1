...
. Als Framework werden wir auf Springboot zurückgreifen. Mit der einfachen Syntax ist Springboot optimal geeignet für kleine bis mittlere Projekte. Bei der Nutzung ist vieles automatisiert (XML Dateien) und ebenfalls sehr einfach aufzusetzen. Es ist modern, für unseren Bedarf sehr flexibel einsetzbar und durch die Vorkenntnisse in Maven das passende Framework für unsere App.
...
. Als Datenbankmanagementsystem nutzen wir Microsoft SQL. Durch Vorerfahrungen mit diesem System wird es erleichtert, die gewünschten Daten zu erhalten und zu verarbeiten.
