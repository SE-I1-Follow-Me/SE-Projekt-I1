= Architecture Notebook: SE I - Belegabgabe
...
.. Das Hauptaugenmerk liegt bei unserem Projekt auf der Follow-me Funktion der Roboter. Folglich wurde der Ausarbeitung dieser Funktion eine sehr hohe Priorität zugeordnet (siehe Vision). Ohne der Follow-me Funktion ist unsere App nicht dem Zweck entsprechend, weshalb diese besonders ausgiebig ausgearbeitet und getestet werden soll.
.. Den Routenoptionen wurde eine hohe Priorität zugeordnet, denn diese sind eng verknüpft mit der Follow-me Funktion. Das Aufzeichnen, Senden und Laden einer Route sind essenziell wichtig, damit Roboter die gewünschten Aufgaben übernehmen können.
