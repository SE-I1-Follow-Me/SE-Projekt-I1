...
.. Das Hauptaugenmerk liegt bei unserem Projekt auf der Follow-me Funktion der Roboter. Folglich wurde der Ausarbeitung dieser Funktion eine sehr hohe Priorität zugeordnet (siehe Vision). Ohne die Follow-me Funktion ist unsere App nicht dem Zweck entsprechend, weshalb diese besonders ausgiebig ausgearbeitet und getestet werden soll.
