...
. Die Roboter werden während ihrer Arbeit gespeer, das heißt, dass andere Kunden, die normalerweise Zugriff auf diese Roboter haben, nicht auf diese zugreifen können, bis der aktuelle Kunde seine Sitzung beendet hat. 
...
. Die Daten über den absolvierten Pfad sollen auf einem Server gespeichert werden. Die App muss daher auf diesen zugreifen. Hierbei wird es einen Server mit X Instanzen geben, damit gewährleistet ist, dass möglichst auch Kunden gleichzeitig auf den Server zugreifen können.
