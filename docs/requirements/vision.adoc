...
| die Auswirkung davon ist | Automatisierte Steuerung der Fahrroboter ist nicht möglich
...
. Ein Kunde kann mehrere Roboter speichern und gleichzeitig bedienen und ein Roboter kann mehreren Kunden gehören, jedoch kann er nur von einem auf einmal benutzt werden bzw. Aufgaben erhalten.
. Die Roboter werden während ihrer Arbeit gesperrt werden, das heißt, dass andere Kunden, die normalerweise Zugriff auf diese Roboter haben, nicht auf diese zugreifen können, bis der aktuelle Kunde seine Sitzung beendet hat. 
...
. Die Daten über den absolvierten Pfad sollen auf einem Server gespeichert werden. Die App muss daher auf diesen zugreifen. Hierbei wird es einen Server mit einer Instanz geben, die Anzahl der Instanzen soll jedoch angepasst werden können.
...
| Route aufzeichnen | hoch | Route abgehen und dabei aufzeichnen | xx
|Route an Server senden |hoch |Aufgenommene Route an den Server senden und auf diesem speichern |
|Route laden |hoch |Zuvor aufgenommene Route anzeigen |
|Route bearbeiten |hoch |Namen der Route ändern oder sie für andere Roboter zugänglich machen |
|Route löschen |hoch |Löschen einer zuvor aufgenommenen Route |
|Follow-Me Funktion |hoch |bei der Follow-Me Funktion handelt es sich um eine erweiterung der "Route aufzeichnen" Funktion, bei der die Route aufgezeichnet wird, während ein oder mehrere Roboter dem Kunden folgen| 
|Roboter hinzufügen |hoch |Speichern eines neuen Roboters |
|Roboter bearbeiten |niedrig |Namen des Roboters ändern | 
|Roboter Fahrkommandos |niedrig |Befehle auf aufgezeichneten Routen an den Roboter geben |
|Roboter löschen |niedrig |Roboter aus der Liste der gespeicherten Roboter entfernen |
|Roboter in Wartungsmodus setzen |sehr niedrig |Roboter wird in Warungsmodus gesetzt; er steht damit bis zur Deaktivierung des Modus nicht zur Verfügung |
|===
...
| App muss auf allen aktuellen Android Versionen lauffähig sein | hoch | xx
|Im Falle eines Absturzes werden die vollständig aufgenommenen Daten gespeichert |hoch | 
|Da die App auch nach Fertigstellung weiter entwickelt werden soll muss im Code möglichst auf hartcodierung verzichtet werden |hoch |
|Das Speichern der Daten auf dem Server soll bei stabiler Internetverbindung nicht länger als 5 Sekunden dauern |mittel |
|Die App muss so entwickelt werden, dass eine Sprachauswahl möglich ist |mittel |
|In der Sprachauswahl soll neben Deutsch auch Englisch verfügbar sein |niedrig |
|===
