...
Falls in Schritt 2 ausgewählt wird, dass die Route nicht abgespeichert werden soll:
...
==== Nutzer bricht das Use Case ab
Sollte der Nutzer das Use Case abbrechen, wird die Route nicht gespeichert.
...
Bestätigung an den Nutzer über die Anwendung gegeben.
