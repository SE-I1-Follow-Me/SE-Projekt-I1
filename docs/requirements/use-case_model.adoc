= Use-Case Model: {project-name}
Vorname Nachname <email@domain.org>; Vorname2 Nachname2 <email2@domain.org>; Vorname3 Nachname3 <email3@domain.org>
{localdatetime}
include::../_includes/default-attributes.inc.adoc[]
// Platzhalter für weitere Dokumenten-Attribute


== Allgemeine Informationen
// ...


== Identifizierte Use Cases
* *UC01*: Anlegen eines Roboters
* *UC02*: Bearbeiten eines Roboters
* *UC03*: Löschen eines Roboters
* *UC04*: Starten der Follow-me Funktion
* *UC05*: Beenden der Follow-me Funktion
* *UC06*: Speichern einer Route
* *UC07*: Bearbeiten einer Route
* *UC08*: Löschen einer Route
* *UC09*: Ablaufen einer Route
* *UC10*: Warten eines Roboters

== Priorisierung der Use Cases
[%header, cols="2,4,1"]
|===
| UC-ID | Use Case Name | Priorität (1-5)
| UC01 | Anlegen eines Roboters | 5
| UC04 | Starten der Follow-me Funktion| 5
| UC05 | Beenden der Follow-me Funktion | 5
| UC06 | Speichern einer Route | 3
| UC09 | Ablaufen einer Route | 2
| UC02 | Bearbeiten eines Roboters | 1
| UC03 | Löschen eines Roboters | 1
| UC07 | Bearbeiten einer Route | 1
| UC08 | Löschen einer Route  | 1
| UC10 | Warten eines Roboters | 1
|===



== Use Case Diagramm
// ...

.Use Case Model PlantUML
[plantuml, "{diagramsdir}/syntax-example2", svg]
....
include::Follow-me Use Case model.puml[]
....


//Fügen Sie nachfolgend alle detaillierten Use-Cases per Include ein.
//Dafür sollten Sie pro Use-Case eine Datei anlegen, die auf dem use_case_01.inc.adoc Template basiert.

////
Mit `<<<` wird im PDF ein Seitenumbruch erzeugt.
Zwischen `include::...` und dem darauf folgenden `<<<` muss sich eine Leerzeile befinden. Fehlt diese, werden unter Umständen die erste Überschriften aus der inkludierten Datei nicht korrekt übernommen.
Ein `include` entfernt am Anfang und Ende eine möglich Leerzeile, was in Kombination mit `<<<` zu Fehlern führen kann.
////


<<<
include::Use Case 01 - Anlegen eines Roboters.adoc[]

<<<
include::Use Case 04 - Starten der Follow-me Funktion.adoc[]

<<<
include::Use Case 05 - Beenden der Follow-me Funktion.adoc[]

//<<<
//include::use_case_01.inc.adoc[]

//<<<
//include::use_case_02.inc.adoc[]

//<<<
//include::use_case_03.inc.adoc[]

//<<<
//include::use_case_04.inc.adoc[]

//<<<
//include::use_case_05.inc.adoc[]

//<<<
//include::use_case_06.inc.adoc[]

//<<<
//include::use_case_07.inc.adoc[]

//<<<
//include::use_case_08.inc.adoc[]

//<<<
//include::use_case_09.inc.adoc[]

//<<<
//include::use_case_10.inc.adoc[]
