...
NOTE: Die funktionalen Anforderungen, die sich aus der Interaktion von Nutzern mit dem System ergeben, sind als Use Cases in einem separaten Dokument festgehalten. link:use-case_model[Use Cases]
...
Wenn ein aufgenommenener GPS-Track hochgeladen wird, ist dieser dauerhaft verfügbar.
SWFA-2: ::
...
NFAR-2: ::
* Nur Roboter der organisationszugehörigen Flotte sollen genutzt werden können.
...
NFAI-1: ::
* Text sollte auf dem Smartphone aus 50cm Entfernung lesbar sein. Zur Überprüfung laden sich Teammitglieder die App herunter und testen die Lesbarkeit.
