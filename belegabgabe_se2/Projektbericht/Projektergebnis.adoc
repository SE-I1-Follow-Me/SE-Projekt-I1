...
=== Produktergebnis
Das Produkt ist leicht weiterentwickelbar. Es wurde bereits zu Beginn des Projektes festgestellt, dass diese App nicht im Verlauf der beiden Semester fertig gestellt werden kann. Dies lag am beträchtlichen Umfang sowie der Größe des Projektteams. 

Die App kann im aktuellen Zustand folgende Features aufweisen: 

* Abrufen der Daten vom Datenbankserver (CRUD)
* Integrierte Karte (Streetview)
...
Das Projekt war eine sehr lehrreiche und intensive Erfahrung für alle Teammitglieder. Trotz entstandener Probleme war die Resonanz der sowohl der Teammitglieder als auch des Kunden überwiegend positiv. Das Projektteam ist stolz auf das Erreichte, auch wenn die App nicht fertig gestellt werden konnte. Da das Projekt so umfangreich war, konnten alle Teammitglieder viel lernen. 

== Reflexionen der Teammitglieder

=== Magnus Andreas Böhne
__Teamrollen: Projektleiter, Serveradministrator, Architekt_
